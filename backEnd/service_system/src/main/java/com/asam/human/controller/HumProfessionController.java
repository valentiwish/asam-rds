...
@Slf4j
public class HumProfessionController extends BaseController {

    @Autowired
    private HumProfessionService humProfessionService;

    @ApiOperation(value = "获取分页列表")
    @RequestMapping("/list")
    public Object list(JPAPage varBasePage, String name) {
        IPage<HumProfession> page = humProfessionService.findPageList(varBasePage, name);
        return R.ok(page);
    }
    ...
    public Object save(String data) {
        if (ObjectUtil.isNotEmpty(data)) {
            HumProfession model = JSON.parseObject(data, HumProfession.class);
            model.setState(Constant.STATE_VALID);
            UserVO curUser=getCurUser();//当前创建人
            boolean flag = humProfessionService.save(model, curUser.getId(), curUser.getUserName());
    ...
    public Object delete(Long id){
        if (ObjectUtil.isNotEmpty(id)) {
            HumProfession model = humProfessionService.getById(id);
    ...
    public Object check(String name,Long id) {
        if (ObjectUtil.isNotEmpty(name)) {
            List<HumProfession> list = humProfessionService.findList(id, name);
    ...
    public Object selectList(){
        List<HumProfession> list = humProfessionService.findList(null,null);
        return R.ok(list);
    }
}
