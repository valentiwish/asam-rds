...
@Slf4j
public class  HumPositionController extends BaseController {

    @Autowired
    private HumPositionService humPositionService;

    @ApiOperation(value = "获取分页列表")
    @RequestMapping("/list")
    public Object list(JPAPage varBasePage, String name) {
        IPage<HumPosition> page = humPositionService.findPageList(varBasePage, name);
        return R.ok(page);
    }
    ...
    public Object save(String data) {
        if (ObjectUtil.isNotEmpty(data)) {
            HumPosition model = JSON.parseObject(data, HumPosition.class);
            model.setState(Constant.STATE_VALID);
            UserVO curUser=getCurUser();//当前创建人
            boolean flag = humPositionService.save(model, curUser.getId(), curUser.getUserName());
    ...
    public Object delete(Long id){
        if (ObjectUtil.isNotEmpty(id)) {
            HumPosition model = humPositionService.getById(id);
    ...
    public Object check(String name,Long id){
        if (ObjectUtil.isNotEmpty(name)) {
            List<HumPosition> list = humPositionService.findList(id, name);
    ...
    public Object selectList(){
        List<HumPosition> list = humPositionService.findList(null,null);
        return R.ok(list);
    }
}
