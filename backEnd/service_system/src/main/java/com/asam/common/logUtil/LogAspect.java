...
import com.asam.feign.IServiceLog;
...
public class LogAspect {
     ...
     * @param args
     */
    public void saveLog(String userName, String url, String ip, String category, OpsLogType type, String desp, Map<String, String[]> args, Integer opResult){
        // 保存日志
        OpsLogVo sysOpsLog = new OpsLogVo();
        sysOpsLog.setOpUserName(userName);
        sysOpsLog.setIp(ip);
        sysOpsLog.setCategory(OpsLogVo.CATEGORY_SYSTEM);
...
//            }
    ...
    @AfterReturning(value = "@annotation(com.asam.common.logUtil.ControllerLog)", returning = "ret")
        ...
        try {
            ...
            System.out.println("获取全路径（协议类型：//域名/项目名/命名空间/action名称?其他参数）url1 = " + url1);*/
                ...
                if (ObjectUtils.isNotEmpty(result)) {
                    if ("200".equals(result.get("code"))||result.get("code").equals(200)) {
                        opResult = 1;
                    }
                }
                ...
                this.saveLog(userName, url, ip, operation.category(), operation.type(), desp, params, opResult);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
