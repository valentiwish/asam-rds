<template>
    <div>
        <page-title>Modbus设备</page-title>
        <div class="page-search">
            <div class="page-search-title">查询条件</div>
            <div class="page-search-content">
                <Form :model="modbusEquipment" :label-width="100" ref="searchForm">
                    <FormItem label="设备编号" prop="equipmentCode">
                        <Input v-model.trim="modbusEquipment.equipmentCode" placeholder="请输入设备编号，不超过50字"
                            maxlength="50"></Input>
                    </FormItem>
                    <FormItem label="设备名称" prop="equipmentName">
                        <Input v-model.trim="modbusEquipment.equipmentName" placeholder="请输入设备名称，不超过50字"
                            maxlength="50"></Input>
                    </FormItem>
                    <FormItem label="相邻站点" prop="adjacentSite">
                        <Input v-model.trim="modbusEquipment.adjacentSite" placeholder="请输入相邻站点名称，不超过50字"
                            maxlength="50"></Input>
                    </FormItem>
                    <FormItem label="设备IP" prop="equipmentIp">
                        <Input v-model.trim="modbusEquipment.equipmentIp" placeholder="请输入设备IP，不超过50字"
                            maxlength="50"></Input>
                    </FormItem>
                </Form>
            ...
            <!-- 新增-->
            <Modal v-model="modal_add" title="新增Modbus设备" :closable="true" :styles="{ width: '50%' }">
            ...
            <!-- 更新-->
            <Modal v-model="modal_update" title="修改Modbus设备" :closable="true" :styles="{ width: '50%' }">
            ...
            <!-- 查看-->
            <Modal v-model="modal_view" title="查看Modbus设备" :closable="true" :styles="{ width: '50%' }">
...
export default {
    ...
    data: function () {
        ...
        return {
            ...
            "gridOption": {
                "auto": false,//是否自动刷新
                "url": "/service_rms/modbusEquipment/list",
                "columns": [ //列配置
                    {
                        "title": "设备编号",
                        "key": "equipmentCode",
    ...
    methods: {
        ...
        del: function (obj) {
            ...
            this.$Modal.confirm({
                ...
                onOk: function () {
                    var idValue = obj.id;
                    var url = "/service_rms/modbusEquipment/delete";
        ...
        search: function () {
            this.$refs.grid.load({ data: JSON.stringify(this.modbusEquipment), equipmentType: this.getEquipmentType() });
        },
        reset: function () {
        ...
        toConnect: function (obj) {
            this.modbusEquipment = obj;
            let robotIp = obj.robotIp;
            let url = "/service_rms/modbusEquipment/toConnect";
            this.$ajax.post(url, { ip: robotIp })
                .then((res) => {
                    if (res.data.code != 200) {
                        this.$Message.error(res.data.msg);
                        this.modbusEquipment.robotConnectState = 0;
                    } else {
                        this.$Message.success("连接成功");
                        this.modbusEquipment.robotConnectState = 1;
                    }
                    //存入最新状态
                    let urlSave = "/service_rms/modbusEquipment/save";
                    this.$ajax.post(urlSave, { "data": JSON.stringify(this.modbusEquipment) })
                        .then((res) => {
                            if (res.data.code == "200") {
        ...
        checkConnect: function () {
            let url = "/service_rms/modbusEquipment/checkConnect";
