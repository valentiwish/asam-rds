<template>
    <div>
        ...
        <div class="page-search">
            ...
            <div class="page-search-content">
                <Form ref="s" v-model="s" :label-width="80"  @submit.native.prevent="submit">
                    <FormItem label="名称">
                        <Input v-model.trim="s.name" maxlength="50" placeholder="请输入名称"></Input>
                    </FormItem>
                    <FormItem label="职责">
                        <Input v-model.trim="s.duty" maxlength="50" placeholder="请输入职责"></Input>
                    </FormItem>

                    <Form-item label="所属部门">
                        <Cascader :data="orgTree" v-model="orgIds" placeholder="请选择所属部门" change-on-select></Cascader>
                    </Form-item>
                </Form>
        ...
        <!-- 新增属性-->
        <Modal v-model="modal_add" title="新增岗位信息">
            <update-modal ref="addform" :orgTreeData="orgTreeData"></update-modal>
            <div slot="footer">
        ...
        <!-- 更新属性-->
        <Modal v-model="modal_update" title="更新岗位信息">
            <update-modal ref="updateform" :id="id" :orgTreeData="orgTreeData"></update-modal>
            <div slot="footer">
        ...
        <!-- 查看属性-->
        <Modal v-model="modal_view" title="查看岗位信息" >
            <update-modal ref="viewform" :id="viewid" type="view"></update-modal>
            <div slot="footer">
    ...
    export default {
        ...
        methods: {
            addModal: function() {
                ...
                this.$refs.addform.loadOrgTree(this.orgTreeData);
                this.modal_add = true;
            },
            //前三个为“行数据”中的方法
            toView: function(obj) {
                this.viewid = obj.id;
                this.$refs.viewform.loadOrgTree(this.orgTreeData);
                this.modal_view = true;
            },
            toUpdate: function(obj) {
                this.id = obj.id;
                this.$refs.updateform.loadOrgTree(this.orgTreeData);
            ...
            del: function(obj) {
                this.$Modal.confirm({
                    ...
                    content: '您确认删除此条数据？',
                    onOk: () => {
                        this.$ajax.post("/service_user/post/delete", { 'id': obj.id })
                        .then((res) => {
            ...
            search: function() {
                ...
                if (this.orgIds && this.orgIds.length > 0){
                    orgId = this.orgIds[this.orgIds.length - 1];
                }
                this.$refs.grid.load({ 'name': this.s.name, 'orgId': orgId,'duty': this.s.duty});
            },
            reset: function() {
                this.s.name = "";
                this.orgIds = [];
