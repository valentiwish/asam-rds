        ...
        <Form :label-width="108" :class="{'form-view':true}" inline>
            <Card>
                ...
                <Form-item label="操作模块">
                    <template>{{obj.operModul}}</template>
                </Form-item>

                <Form-item label="操作类型">
                    <template>{{obj.operType}}</template>
                </Form-item>

                <Form-item label="操作描述">
                    <template>{{obj.operDesc}}</template>
                </Form-item>
                ...
                <Form-item label="传入参数" class="singleline">
                    <template>{{obj.operRequParam}}</template>
                </Form-item>

                <Form-item label="返回参数" class="singleline">
                    <template>{{obj.operRespParam}}</template>
                </Form-item>
            </Card>
        </Form>
    </div>
...
<script>
    export default {
        ...
        methods: {
            //查询数据
            getData: function(id) {
                this.$ajax.post("/service_user/sysLog/findOpeById", {"id": id})
                .then((res) => {
