        ...
        <div class="page-search">
            ...
            <div class="page-search-content">
                <Form :label-width="100" @submit.native.prevent="search">
                    <FormItem label="类型">
                        <Select v-model="mobileVersion.type" placeholder="请选择app类型">
                    ...
                    <FormItem label="版本名称"> <Input v-model.trim="mobileVersion.name" placeholder="请输入版本名称，最多输入20字" maxlength="20"></Input></FormItem>
                    <FormItem label="版本号"> <Input v-model="mobileVersion.number" placeholder="请输入版本号"></Input></FormItem>
                </Form>
                <div class="search-btn">
                    ...
                    <allowBtn allow="新增" type="success" icon="plus" @click="mv_add=true">新增版本</allowBtn>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        data: function() {
            ...
            return {
                ...
                templateTypeList:[],
                "gridOption": {
                    "url":"/service_user/mobileVersion/list",
                    "data": [],
                    "columns": [
                        {
                            "title": "类型",
        ...
        methods: {
            ...
            del: function(obj) {
                this.$Modal.confirm({
                    ...
                    content: '您确认删除此条数据？',
                    onOk: () => {
                        this.$ajax.post("/service_user/mobileVersion/delete", { 'id': obj.id })
                        .then((res) => {
            ...
            search: function() {
                var startTime = "";
                this.$refs.grid.load({
                    "name": this.mobileVersion.name, "number": this.mobileVersion.number,"type": this.mobileVersion.type
                });
            },        
            reset: function() {
                this.mobileVersion.name = "";
                this.mobileVersion.number = "";
                this.mobileVersion.type = "";
            },
            //以下二个是子页面按钮的方法
            save: function(){
                this.loading = true;
        ...
        created: function() {             
            this.$ajax.post("/service_user/sysData/findByTypeCode",{"dataTypeCode":"APP_NAME"})
                .then((res) => {
